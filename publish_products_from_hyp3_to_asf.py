...
def generate_ingest_message(hyp3_job_dict: dict, response_topic_arn: str) -> dict:
...
def get_cmr_product_ids(cmr_domain: str, collection_short_name: str) -> list[str]:
...
def publish_messages(messages: list, topic_arn: str, dry_run: bool) -> None:
...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument('--job-type', default='INSAR_ISCE', choices=['INSAR_ISCE', 'ARIA_RAIDER'])
    parser.add_argument('--start', type=parse_datetime)
    parser.add_argument('--collection-short-name', default='ARIA_S1_GUNW', choices=['ARIA_S1_GUNW'])
    parser.add_argument(
        '--hyp3-urls',
        nargs='+',
        default=[
            'https://hyp3-a19-jpl.asf.alaska.edu',
            'https://hyp3-tibet-jpl.asf.alaska.edu',
